"""Paste a Nano Banana Pro edit of one region back into a photo (used for site/assets/room.jpg), leaving
the rest of the frame unchanged apart from the JPEG re-encode.
...
try:
    ...
    if len(box) != 4 or feather < 0: raise ValueError
except ValueError:
    die("X, Y, N and feather must be integers (feather not negative) and the box four comma-separated integers")

room = Image.open(src).convert("RGB")
edit = Image.open(edit_path).convert("RGB").resize((n, n), Image.LANCZOS)
if n <= 0 or ox < 0 or oy < 0 or ox + n > room.width or oy + n > room.height:
    die(f"the crop {n}x{n} at ({ox},{oy}) does not fit the {room.width}x{room.height} image")
bl, bt, br, bb = box
if not (0 <= bl < br <= n and 0 <= bt < bb <= n):
    die(f"the box {bl},{bt},{br},{bb} is not a rectangle inside the {n}x{n} crop")
if bl <= 20 and bt <= 20 and br >= n - 20 and bb >= n - 20:
    die("the box plus its 20px margin covers the whole crop; no pixels are left to align on")
...
def shifted(dx, dy):
    """The edit moved by (dx, dy). The band it vacates at the crop edge shows the original photo, not a
    wrap-around of the edit's far side, so a box that touches the edge pastes nothing wrong."""
    img = orig.copy(); img.paste(edit, (dx, dy)); return img
search = 8
base = score(edit)
s, dx, dy = min((score(shifted(dx, dy)), dx, dy) for dx in range(-search, search + 1) for dy in range(-search, search + 1))
print(f"outside-box mean diff {base:.2f} -> {s:.2f} at offset ({dx},{dy})")
# A best offset on the search boundary means the true offset is outside the window: refuse rather than
# composite a misaligned patch.
